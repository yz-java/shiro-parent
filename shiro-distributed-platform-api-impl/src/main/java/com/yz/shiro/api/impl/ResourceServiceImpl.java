...
import com.yz.shiro.dao.ResourceMapper;
...
public class ResourceServiceImpl implements ResourceService {
    ...
    public boolean createResource(Resource resource) {
        ...
        if (b) {
            redisTemplate.opsForHash().put(ResourceService.CACHE_FLAG, resource.getId().toString(), resource);
            return true;
        }
        return false;
    }
    ...
    public List<Resource> findAll() {
        Set keys = redisTemplate.opsForHash().keys(ResourceService.CACHE_FLAG);
        List<Resource> resources = redisTemplate.opsForHash().multiGet(ResourceService.CACHE_FLAG, keys);
        return resources;
    }
    ...
    public List<Resource> getByIds(List<Long> ids) {
        ...
        List<Resource> resources = redisTemplate.opsForHash().multiGet(ResourceService.CACHE_FLAG, list);
    ...
    public void cacheAllResources() {
        ...
        redisTemplate.opsForHash().putAll(ResourceService.CACHE_FLAG, resourceMap);
    }
    ...
    private void cacheReload(List<Resource> resources) {
        ...
        if (!CollectionUtils.isEmpty(resources)) {
            redisTemplate.opsForHash().putAll(ResourceService.CACHE_FLAG, cacheMap);
        }
    }
